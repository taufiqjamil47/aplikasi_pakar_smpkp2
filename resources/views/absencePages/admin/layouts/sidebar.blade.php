    ...
    <div class="p-4">
        <nav class="mt-6">
            <div class="space-y-1">
                ...
                @endif
                @if (auth()->user()->hasAnyRole([1, 4]))
                    <a href="{{ route('attendance.export.index') }}"
                        class="nav-item flex items-center px-4 py-3 text-sm font-medium text-gray-600 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors duration-200 {{ Request::is('export') ? 'active' : '' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3" viewBox="0 0 20 20"
                            fill="currentColor">
                            <path fill-rule="evenodd"
